...
TaskclusterContainer.prototype._getArtifact = function() {
    if(!this.artifact) {
        this.artifact = this._getArtifacts().then((artifacts) => {
            return artifacts.find((artifact) => artifact.name.includes(this.fileEnding) && (!this.path || artifact.name.includes(this.path)));
        });
    }
