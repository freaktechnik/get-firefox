...
TaskclusterContainer.prototype.getFileURL = function() {
    return this._getArtifacts().then((artifacts) => {
        const artifactName = artifacts.find((a) => a.name.indexOf(this.fileEnding) != -1).name;
...
TaskclusterContainer.prototype.getFileName = function() {
    return this._getArtifacts().then((artifacts) => {
        const artifact = artifacts.find((a) => a.name.indexOf(this.fileEnding) != -1);
...
TaskclusterContainer.prototype.getChecksums = function() {
    return this._getArtifacts().then((artifacts) => {
        const artifactName = artifacts.find((a) => a.name.indexOf(".checksums") != -1).name;
        ...
        if(response.ok) {
            ...
            throw "Could not load checksums from " + response.url + ":" + response.statusText;
        }
    });
};
